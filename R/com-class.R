...
setClass(
  ...
  representation(
    ...
    strata = "list", item_values = "list", source_ref = "character"
  ),
  prototype(
    ...
    strata = list(), item_values = list(), source_ref = NA_character_
  ),
...
#'
#' @param id,name,version,items,item_ranges,item_type,subscales,aggregation,direction,strata,item_values,source_ref
...
ClinicalInstrument <- function(id, name = NA_character_, version = NA_character_,
  ...
  new("ClinicalInstrument", id = as.character(id), name = as.character(name),
      ...
      item_values = item_values, source_ref = as.character(source_ref))
}
